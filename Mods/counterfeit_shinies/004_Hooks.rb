...
Events.onEndBattle += proc { |_sender, e|
  begin
    next if !$PokemonTemp || !$PokemonTemp.counterfeit_shiny_context
    decision = e.is_a?(Array) ? e[0] : e
    $PokemonTemp.counterfeit_shiny_context[:battle_decision] = decision.to_i
  rescue
  end
}
