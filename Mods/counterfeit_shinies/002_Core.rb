...
module CounterfeitShinies
  ...
  def forge_cost
    return Config::FORGE_COST.to_i
  end

  def can_afford_forge_cost?
    return true if forge_cost <= 0
    return true if !$Trainer
    return $Trainer.money.to_i >= forge_cost
  end

  def charge_forge_cost!
    return true if forge_cost <= 0
    return true if !$Trainer
    return false if !can_afford_forge_cost?
    $Trainer.money -= forge_cost
  ...
  def confiscate_party_counterfeits_on_loss!
    confiscated = []
    stripped = []
    party_sale_candidates.each do |ref|
      pokemon = ref[:pokemon]
      next if !pokemon || !counterfeit?(pokemon)
      if Config::STRIP_IF_LAST_OWNED && !safe_to_remove_entire_pokemon?(pokemon)
        next if !strip_instead_of_confiscating!(pokemon)
        stripped << pokemon
      elsif remove_owned_pokemon(pokemon, :confiscated)
        confiscated << pokemon
      end
    end
    return {
  ...
  def engage_enforcer_battle(context, opening_lines = nil, battle_from_chase = false)
    ...
    $PokemonTemp.counterfeit_shiny_context[:type] = :enforcer
    $PokemonTemp.counterfeit_shiny_context[:battle_origin] = battle_from_chase ? :chase : :direct
    $PokemonTemp.counterfeit_shiny_context[:battle_decision] = nil
    ...
    if $PokemonTemp && $PokemonTemp.counterfeit_shiny_context &&
       $PokemonTemp.counterfeit_shiny_context[:type] == :enforcer
      effective_decision = $PokemonTemp.counterfeit_shiny_context[:battle_decision]
      effective_decision = decision if effective_decision.nil?
      handle_enforcer_battle_end(effective_decision)
    end
    $PokemonTemp.counterfeit_shiny_context = nil if $PokemonTemp && decision.to_i == 0
    return decision.to_i == 1
  end

  def clear_stale_enforcer_context!
    ...
    return if enforcer_chase_active?
    return if $game_temp && $game_temp.message_window_showing
    return if pbMapInterpreterRunning?
    return if !context[:resolved] && context[:battle_decision].nil?
    $PokemonTemp.counterfeit_shiny_context = nil
  end
  ...
  def handle_enforcer_battle_end(decision)
    ...
    context = $PokemonTemp.counterfeit_shiny_context
    return if context[:type] != :enforcer
    return if context[:resolved]
    state = global_state
    decision = context[:battle_decision] if !context[:battle_decision].nil?
    decision = decision.to_i
    context[:resolved] = true
    state[:last_enforcer_decision] = decision
    if decision == 1
      state[:wins] += 1
      ...
      result = confiscate_party_counterfeits_on_loss!
      confiscated = result[:confiscated]
      stripped = result[:stripped]
      state[:last_confiscated] = confiscated.last.species if !confiscated.empty?
      if confiscated.length == 1 && stripped.empty?
        pbMessage(_INTL("The enforcer confiscates {1} as evidence and vanishes into the crowd.", confiscated.first.name))
      elsif confiscated.length > 1
        pbMessage(_INTL("The enforcer sweeps up the counterfeit stock still in your party and vanishes into the crowd."))
      end
      if stripped.length == 1
        pbMessage(_INTL("The enforcer strips the counterfeit finish from {1} and lets the Pokemon go.", stripped.first.name))
      elsif stripped.length > 1
        pbMessage(_INTL("The enforcer strips the forged finishes they cannot carry and leaves the Pokemon behind."))
      end
    end
  ensure
