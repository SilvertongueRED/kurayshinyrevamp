...
class CounterfeitShinyEditorScene
  ...
  def slider_items
    ...
    commit_value = if @new_counterfeit && CounterfeitShinies.forge_cost > 0
                     CounterfeitShinies.format_money(CounterfeitShinies.forge_cost)
                   else
                     "Commit"
                   end
    return [
      ...
      { :label => (@new_counterfeit ? "Forge" : "Save"), :value => commit_value, :label_color => Color.new(80, 160, 80) },
  ...
  def handle_action
    ...
    when SAVE_FAVORITE_INDEX
      ...
      return :continue
    when COMMIT_INDEX
      if @new_counterfeit && !CounterfeitShinies.charge_forge_cost!
        pbMessage(_INTL("You need ${1} in parts and solvent to forge a new finish.", CounterfeitShinies.forge_cost.to_s_formatted))
        return :continue
      end
