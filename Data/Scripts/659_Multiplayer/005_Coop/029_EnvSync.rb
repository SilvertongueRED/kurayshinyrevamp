...
module MPEnvSync
  ...
  # User preference (Multiplayer Settings -> "Sync Day/Wx"). Defaults ON.
  def env_pref_on?
    v = ($PokemonSystem.mp_env_sync rescue 1)
    v.nil? ? true : v.to_i != 0   # default ON: a nil pref (older save) must not disable sync
  rescue
    true
  end
  ...
  def set_time_from_leader(leader_epoch)
    ...
    return if le <= 0
    # The offset is ADDED to pbGetTimeNow's result, so it must be computed in the
    # SAME clock domain pbGetTimeNow returns. With the UnrealTime add-on active
    # (052_AddOns/UnrealTime.rb, ENABLED=true here), pbGetTimeNow is an in-game
    # accelerated clock (epoch ~year 2000), NOT real Time.now -- so basing the
    # offset on Time.now shifted members ~25 years and landed them on the wrong
    # hour ("the leader's time of day is different than what I switched to"). Base
    # the offset on the UNDERLYING clock (UnrealTime if present, else real
    # Time.now) so the member's pbGetTimeNow equals the leader's exact in-game time.
    base = (mpenv_orig_pbGetTimeNow.to_f rescue Time.now.to_f)
    @time_offset = le - base
    @time_active = true
  ...
  def pump
    # Respect the user's "Sync Day/Wx" toggle; revert to our own clock when off.
    unless env_pref_on?
      clear if @time_active
      return
    end
