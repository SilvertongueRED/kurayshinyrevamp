...
module CoopTargetIntent
  ...
  @hover_ctx = nil  # [attacker_idx, target_data] while an interactive target pick is open

  module_function

  # User preference (Multiplayer Settings -> "Ally Arrow"). Defaults ON. When OFF
  # we neither broadcast our own hover nor ingest/draw a teammate's.
  def arrow_pref_on?
    v = ($PokemonSystem.mp_ally_arrow rescue 1)
    v.nil? ? true : v.to_i != 0   # default ON: a nil pref (older save) must not hide the arrow
  rescue
    true
  end
  ...
  def prune_stale(battle)
    ...
    @remote_intents.reject! { |_idx, info| t = info[:turn].to_i; t < cur || t > cur + 1 }
  rescue
    nil
  end
  ...
  def broadcast_hover(battle, attacker_idx, hovered_idx, target_data)
    ...
    # Any live battler may be hovered now -- foes AND player-side Pokemon
    # (self / squadmate), so allies also see a marker for support moves
    # (heals, Helping Hand, etc.) aimed at their own side.
    hb = (battle.battlers[hovered_idx] rescue nil)
    return unless hb && !(hb.fainted? rescue true)
  ...
  def active_targets(battle)
    ...
    @remote_intents.each_value do |info|
      next unless info[:turn].to_i >= cur && info[:turn].to_i <= cur + 1
      t = info[:target]
      next if t.nil? || t < 0
      out[t] ||= 0
      out[t]  += 1
  ...
  def clear_local_hover
    @local_hover = ""
  end

  # --- Hover context ----------------------------------------------------------
  # Set by the battle-level pbChooseTarget wrapper below for the duration of the
  # interactive target pick. While set, EVERY pbSelectBattler(idx, 2) call -- the
  # one thing all target-selection UIs share (vanilla scene, EBDX scene, and the
  # Mouse UI mod's wholesale overrides of pbChooseTarget) -- broadcasts the
  # hovered foe. The old in-loop hover hooks lived inside the vanilla scene's
  # pbChooseTarget and were silently replaced by the Mouse UI mod, which is why
  # the live arrow never moved; this path survives any such override.
  def begin_hover_context(attacker_idx, target_data)
    @hover_ctx = [attacker_idx, target_data]
  end

  def end_hover_context(confirmed)
    @hover_ctx = nil
    clear_local_hover unless confirmed   # cancelled: stop showing our arrow
  end

  # Called from the pbSelectBattler hook (662/001_TargetMarker.rb).
  def on_select_battler(battle, idx, select_mode)
    return unless select_mode == 2          # 2 == target-selection highlight
    return unless @hover_ctx
    return unless idx.is_a?(Integer) && idx >= 0
    atk, tdata = @hover_ctx
    bid = (CoopBattleState.battle_id rescue nil)
    return if "#{bid}:#{atk.to_i}:#{idx.to_i}" == @local_hover.to_s   # dedupe repeats
    broadcast_hover(battle, atk, idx, tdata)
  rescue
    nil
  end
  ...
  def ingest_sync_hovers(battle)
    ...
    allies.each do |asid|
      ...
      tok = pd[:bhov].to_s
      if tok.empty?
        # Ally cleared their hover (attack phase / cancelled) -- drop their stale
        # intents so the marker doesn't linger on an old target.
        norm = asid.to_s.gsub(/\ASID/i, "")
        @remote_intents.reject! do |_a, info|
          s = info[:sid].to_s.gsub(/\ASID/i, "")
          s == norm || (!s.empty? && !norm.empty? && (s.include?(norm) || norm.include?(s)))
        end
        next
      end
      parts = tok.split(":")
...
class PokeBattle_Battle
  ...
  def pbChooseTarget(battler, move)
    _hover_ctx = false
    if defined?(CoopBattleState) && CoopBattleState.in_coop_battle? && defined?(CoopTargetIntent)
      begin
        CoopTargetIntent.begin_hover_context(battler.index, move.pbTarget(battler))
        _hover_ctx = true
      rescue
      end
    end
    ...
    begin
      ...
      (CoopTargetIntent.end_hover_context(!!ret) rescue nil) if _hover_ctx
    end
    if ret && defined?(CoopBattleState) && CoopBattleState.in_coop_battle? && defined?(CoopTargetIntent)
      begin
        if pbOwnedByPlayer?(battler.index)
          ...
          # Flag any single-target move -- foe-targeted (so teammates don't
          # wastefully pick the same opponent) AND player-side-targeted (so a
          # teammate sees a support move incoming on themselves / their ally).
          single = true
          begin
