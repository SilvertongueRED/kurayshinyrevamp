...
module KIFProfileStatTrackers
  ...
  @last_caught_cache_version = nil
  @pending_caught_count = nil
  ...
  def self.tick_caught
    ...
    cache_version = pokedex.respond_to?(:csf_pokedex_cache_version) ? pokedex.csf_pokedex_cache_version : nil
    return if @pending_caught_count.nil? && !@last_caught_count.nil? && !cache_version.nil? && @last_caught_cache_version == cache_version
    count = (pokedex.owned_count rescue nil)
    return if count.nil?

    now = Time.now.to_f
    @pending_caught_count = count if @last_caught_count != count
    if !@pending_caught_count.nil? && (now - @last_caught_flush) >= CAUGHT_FLUSH_INTERVAL
      @last_caught_count = @pending_caught_count
      @pending_caught_count = nil
      @last_caught_cache_version = cache_version
      @last_caught_flush = now
      MultiplayerClient.send_data("STAT_POKECAUGHT:#{@last_caught_count}") rescue nil
    end
  rescue; end
end
