...
class PokemonLoad_Scene
  ...
  def pbResolveTitlePlayerCharset(filename)
    candidates = []
    candidates << filename if filename && filename.to_s.length > 0
    candidates << "walk"
    candidates << "trchar000"
    candidates.uniq.each do |candidate|
      return candidate if pbResolveBitmap("Graphics/Characters/#{candidate}")
    end
    nil
  end
  ...
  def pbSetParty(trainer)
    ...
    if meta
      ...
      resolved_filename = pbResolveTitlePlayerCharset(filename)
      if resolved_filename
        MultiplayerDebug.warn("TITLE-SPR", "Missing player charset #{filename.inspect}; using #{resolved_filename.inspect}") if defined?(MultiplayerDebug) && resolved_filename != filename
        begin
          @sprites["player"] = TrainerWalkingCharSprite.new(resolved_filename, @viewport, trainer)
          bitmap = @sprites["player"].bitmap
          if bitmap
            charwidth  = bitmap.width
            charheight = bitmap.height
            @sprites["player"].x        = 56 * 2 - charwidth / 8
            @sprites["player"].y        = 56 * 2 - charheight / 8 + y_offset
            @sprites["player"].src_rect = Rect.new(0, 0, charwidth / 4, charheight / 4)
          else
            MultiplayerDebug.warn("TITLE-SPR", "Player charset #{resolved_filename.inspect} loaded without a bitmap; skipping preview") if defined?(MultiplayerDebug)
            @sprites["player"].dispose rescue nil
            @sprites.delete("player")
          end
        rescue => e
          MultiplayerDebug.warn("TITLE-SPR", "Failed to build title player sprite #{resolved_filename.inspect}: #{e.class}: #{e.message}") if defined?(MultiplayerDebug)
          @sprites["player"].dispose rescue nil
          @sprites.delete("player")
        end
      else
        MultiplayerDebug.warn("TITLE-SPR", "No title player charset found for #{filename.inspect}; skipping preview") if defined?(MultiplayerDebug)
      end
    end
