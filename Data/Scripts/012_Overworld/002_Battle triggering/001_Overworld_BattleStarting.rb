...
def pbAfterBattle(decision,canLose)
  ...
  if decision == 4
    recovered_capture_scene = false
    begin
      recovered_capture_scene = CaptureCompletionFallback.flush(true) if defined?(CaptureCompletionFallback)
    rescue
      recovered_capture_scene = false
    end
    if recovered_capture_scene
      begin
